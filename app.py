...
# --------------------------
DB_PATH = os.path.join("/tmp", "sec_filings.db")  # Cloud writable
...
def predict_impact(filing, stock_data):
    ...
    form_impact = {'8-K': 0.2, '10-Q': 0.1, '10-K': 0.1, '4': -0.3, 'SC 13D': 0.4}
...
def monitor_sec_filings():
    ...
    for entry in feed.entries:
        try:
            ...
            cik = str(entry.link.split('CIK=')[-1].split('&')[0]).zfill(10)
            ...
            if not conn.execute('SELECT id FROM filings WHERE id=?', (filing_id,)).fetchone():
                ...
                conn.commit()
                new_filings.append({'timestamp': timestamp, 'form_type': form_type, 'company': company, 'cik': cik, 'filing_link': entry.link})
        except Exception as e:
